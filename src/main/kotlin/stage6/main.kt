...
class Matrix(private var state: Array<Array<Double>>) {
    ...
    fun inverse(): Matrix {
        ...
        if (determinant == 0.0) {
            throw ArithmeticException("Cannot invert matrix because determinant is 0!")
        }
        ...
        for (row in 0 until rowCount) {
            for (col in 0 until colCount) {
                ...
                val j = col + 1
                val sign = (-1.0).pow(i + j)
                newState[row][col] = sign * getSubMatrix(row, col).determinant() / determinant
            }
        }

        return Matrix(newState).transpose { x, y -> Pair(y, x) }
    }
}